...
export default function MovieList({genreId,searchKeyword}) {
    ...
    useEffect(() => {
        ...
        // 0 ==> false,
        // false ==> false
        // genreId가 undefined 가 아니라면
        if (genreId) { 
            setScrollable(true)
            axios.get(`https://api.themoviedb.org/3/discover/movie?api_key=3cc0c46b7fd6181b12a5d89d2a40c0a8&language=ko-KR&region=KR&sort_by=popularity.desc&include_adult=false&with_genres=${genreId}`)
                 .then((response) => {
    ...
    return (
            <ul className= {scrollable? "movieList" : "search_list" }> 
